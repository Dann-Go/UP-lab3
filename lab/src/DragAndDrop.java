 ...
 **/
public class DragAndDrop extends JComponent implements DragGestureListener, // For
        // recognizing
        // the
        // start
        // of
        ...
        MouseMotionListener // For processing mouse drags
{
    ...
    ArrayList<Astroid> leafs = new ArrayList<Astroid>();
    // A list of Scribbles to draw
    Astroid currentScribble; // The scribble in progress
    Astroid beingDragged; // The scribble being dragged
    ...
    public void paintComponent(Graphics g)
    {
        ...
        g2.setStroke(new StrokeToDraw(4)); // Specify wide lines

        int numScribbles = leafs.size();
        for (int i = 0; i < numScribbles; i++)
        {
            Astroid s = leafs.get(i);
    ...
    public void mousePressed(MouseEvent e)
    {
        if (dragMode)
            return;
        currentScribble = new Astroid(240,getWidth()/2, getHeight()/2);
    ...
    public void dragGestureRecognized(DragGestureEvent e)
    {
        ...
        { // Loop through the scribbles
            ...
            if (s.intersects(r))
            {
                ...
                Astroid dragScribble = (Astroid) s.clone();
    ...
    {
        if (e.isDataFlavorSupported(Astroid.decDataFlavor) || e.isDataFlavorSupported(DataFlavor.stringFlavor))
        {
    ...
    public void drop(DropTargetDropEvent e)
    {
        ...
        Astroid droppedScribble; // This will hold the Scribble object

        // First, try to get the data directly as a scribble object
        try
        {
            droppedScribble = (Astroid) t.getTransferData(Astroid.decDataFlavor);
        } catch (Exception ex)
        { // unsupported flavor, IO exception, etc.
            ...
            try
            {
                ...
                droppedScribble = Astroid.getFromString(s);
            } catch (Exception ex2)
            {
